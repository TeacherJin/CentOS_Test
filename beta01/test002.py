...
# 使用正则表达式在指定文件中搜索关键词，file表示文件路径，pattern表示正则表达式
def reSearchKeyword(file,pattern):
    with open(file) as f:
        lines=f.readlines()
        for line in lines:
...
def detectTest():
    ...
    # '^ *touch +F1 *$'表示命令 touch F1，因为空格可以有多个，命令前后均可能出现空格
    # 命令中的空格也可以不止一个
    result[4]=reSearchKeyword(home_path+'/.bash_history','^ *touch +F1 *$') and os.path.exists(home_path+'/dirA/File1')
    
    # 5，搜索命令历史记录，并检测文件是否存在
    result[5]=reSearchKeyword(home_path+'/.bash_history','^ *cp +F2 +testdirB/File2 *$') and os.path.exists(home_path+'/F2') and os.path.exists(home_path+'/testdirB/File2')
    
    # 6，搜索命令历史记录
    result[6]=reSearchKeyword(home_path+'/.bash_history','^ *mkdir +dirB *$') and reSearchKeyword(home_path+'/.bash_history','^ *cp +-[Rr] +dirA +dirB *$')
    
    # 7，搜索命令历史记录
    result[7]=reSearchKeyword(home_path+'/.bash_history','^ *rm +-rf? +dirB *$')
